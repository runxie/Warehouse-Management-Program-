...
orderList = open('warehouse-orders-v02.txt')
...
def calRoute(num1, num2, route):
    if int(orderInput[num1][2]) == int(orderInput[num2][2]):
        route.append([round(orderInput[num2][0],2),round(orderInput[num2][1],2),round(orderInput[num2][2],2)])
...
def opt2(orderInput,f):
    ...
    timecost = end - start
    f.write("bestPath = " + str(bestPath) + "\n")
    f.write("total distance = "+str(distHistory[-1])+"\n")
    ...
    f.write("total effort = "+ str(ttPathEff(bestPath)[1])+"\n")
    ...
    f.write("total effort (reverse) = "+str(ttPathEff(repath)[1])+"\n")
    ...
    f.write("time cost = "+str(timecost)+"\n")
...
if (__name__ == '__main__'):
    ...
    if batchmode:
        ...
        weight = weight.tolist()
        print(weight)
        f = open("BatchFile_CombineOrders.txt", "w")
        count=1
        while len(weight):
            f.write("CombOrders Number " + str(count) + "\n")
            order = []
            for i in range(len(weight) - 1, -1, -1):
                if captmp >= weight[i][1]:
                    captmp -= weight[i][1]
            ...
            captmp = cartcapacity
            # print("Combine orders:", order)
            f.write("Combine orders : " + str(order) + "\n")
            orderInput = orderCombineData(order, orderInput)
